...
export function LoanList({ loans, items, attentionMode = false }: LoanListProps) {
  ...
  return (
    <section className="space-y-5">
      <div className="grid gap-4 sm:grid-cols-3"><Summary label={attentionMode ? "Perlu ditindaklanjuti" : "Transaksi aktif"} value={activeCount} /><Summary label="Terlambat" value={overdueCount} alert /><Summary label="Alat masih dipinjam" value={borrowedQuantity} /></div>
      ...
      <FormDialog open={isOpen} onOpenChange={setIsOpen} title="Buat peminjaman" description="Stok alat otomatis berkurang setelah transaksi disimpan."><LoanForm items={items} onSuccess={(loanId) => { setIsOpen(false); router.push("/loans/" + loanId); }} /></FormDialog>
    </section>
  );
}
...
function LoanForm({ items, onSuccess }: { items: LoanableItem[]; onSuccess: (loanId: string) => void }) {
  ...
  const submit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    if (loanItems.length === 0) return toast.error("Cari lalu pilih minimal satu alat.");
    setIsPending(true);
    try {
