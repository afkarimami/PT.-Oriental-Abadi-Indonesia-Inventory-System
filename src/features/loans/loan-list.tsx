  ...
  return (
    <section className="space-y-5">
      <div className="grid gap-4 sm:grid-cols-3">
        <Summary label={attentionMode ? "Perlu ditindaklanjuti" : "Transaksi aktif"} value={activeCount} />
        <Summary label="Terlambat" value={overdueCount} alert />
        <Summary label="Barang masih dipinjam" value={borrowedQuantity} />
      </div>
              ...
              <Link key={loan.id} href={"/loans/" + loan.id} className="flex items-center gap-4 p-4 transition hover:bg-muted/50">
                <div className="min-w-0 flex-1">
                  <div className="flex items-center gap-2">
                    <p className="font-mono text-xs font-semibold text-primary">{loan.code}</p>
                    <StatusBadge status={loan.status} />
                    {loan.attentionStatus === "due_soon" && <Badge variant="secondary">Tenggat dekat</Badge>}
                  </div>
                  <p className="mt-1 font-semibold">{loan.borrowerName}</p>
                  <p className="text-sm text-muted-foreground">
                    {loan.purpose} - {loan.itemCount} jenis alat, {loan.outstandingQuantity} masih dipinjam
                  </p>
                </div>
                <div className="hidden text-right text-sm sm:block">
                  <p>{loan.expectedReturnOn ? "Kembali " + new Date(loan.expectedReturnOn).toLocaleDateString("id-ID") : "Tanpa jadwal kembali"}</p>
      ...
      <FormDialog open={isOpen} onOpenChange={setIsOpen} title="Buat peminjaman" description="Stok barang otomatis berkurang setelah transaksi disimpan.">
        <LoanForm items={items} onSuccess={(loanId) => { setIsOpen(false); router.push("/loans/" + loanId); }} />
      </FormDialog>
    </section>
  );
}
...
function Summary({ label, value, alert = false }: { label: string; value: number; alert?: boolean }) {
  return (
    <div className={"rounded-2xl border p-4 " + (alert && value > 0 ? "border-amber-200 bg-amber-50" : "bg-card")}>
...
function StatusBadge({ status }: { status: keyof typeof statusLabel }) {
  return <Badge variant={status === "overdue" ? "destructive" : status === "closed" ? "secondary" : "default"}>{statusLabel[status]}</Badge>;
}
  ...
  return (
    <form className="grid gap-4" onSubmit={submit}>
      <Field label="Nama peminjam">
        <Input value={borrowerName} onChange={(event) => setBorrowerName(event.target.value)} required />
      </Field>
      <Field label="Nomor HP">
        <Input type="tel" value={borrowerPhone} onChange={(event) => setBorrowerPhone(event.target.value)} required />
      </Field>
      <Field label="Instansi (opsional)">
        <Input value={borrowerOrganization} onChange={(event) => setBorrowerOrganization(event.target.value)} placeholder="Untuk peminjam dari luar" />
      </Field>
      ...
      <Field label="Rencana kembali (opsional)">
        <Input type="date" value={expectedReturnOn} onChange={(event) => setExpectedReturnOn(event.target.value)} />
      </Field>
      <Field label="Cari barang">
        <div className="relative">
          <Search className="pointer-events-none absolute top-1/2 left-3 z-10 size-4 -translate-y-1/2 text-muted-foreground" />
          <Input value={itemSearch} onChange={(event) => setItemSearch(event.target.value)} className="pl-9" placeholder="Ketik nama atau kode barang" autoComplete="off" />
          {itemSearch && (
              ...
              {searchResults.length === 0 ? (
                ...
                searchResults.map((item) => (
                  <button key={item.id} type="button" onClick={() => addItem(item)} className="flex w-full items-center justify-between gap-3 px-3 py-3 text-left text-sm hover:bg-muted">
                    ...
                    <span className="shrink-0 text-xs text-primary">Tersedia {item.availableQuantity}</span>
                  </button>
                ))
              )}
            </div>
              ...
              <div key={row.inventoryItemId} className="flex items-center justify-between gap-2 rounded-xl border p-3">
                <div className="min-w-0 flex-1">
                  ...
                  <p className="font-mono text-xs text-muted-foreground">
                    {item.code} - Rak {item.rackCode} - tersedia {item.availableQuantity}
                  </p>
                </div>
                      ...
                      setLoanItems((current) =>
                        current.map((selected) =>
                          selected.inventoryItemId === row.inventoryItemId
                            ? { ...selected, quantity: Math.max(1, selected.quantity - 1) }
                            : selected
                        )
                      )
                    }
...
/>
                      ...
                      setLoanItems((current) =>
                        current.map((selected) =>
                          selected.inventoryItemId === row.inventoryItemId
                            ? {
                                ...selected,
                                quantity: Math.min(item.availableQuantity, selected.quantity + 1),
                              }
                            : selected
                        )
                      )
                ...
                <Button type="button" variant="ghost" onClick={() => setLoanItems((current) => current.filter((selected) => selected.inventoryItemId !== row.inventoryItemId))}>
                  Hapus
                </Button>
              </div>
            );
      ...
      <Button type="submit" disabled={isPending}>
        {isPending ? "Menyimpan..." : "Buat peminjaman"}
      </Button>
    </form>
  );
}
