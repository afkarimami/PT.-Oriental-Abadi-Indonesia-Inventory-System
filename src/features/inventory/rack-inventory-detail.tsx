    ...
    <section className="space-y-6">
      ...
      <div className="rounded-2xl border bg-card p-5 shadow-[0_8px_24px_rgba(28,36,52,0.04)]">
        <div className="flex flex-col gap-5 sm:flex-row sm:items-center sm:justify-between">
          <div><p className="text-sm font-semibold text-primary">Rak {rack.code}</p><h2 className="mt-1 text-2xl font-bold">{rack.name} - {rack.locationName}</h2><p className="mt-1 text-sm text-muted-foreground">Daftar barang yang tersusun di rak ini.</p></div>
          <div className="rounded-xl bg-muted/60 px-5 py-3 text-center"><p className="text-xs text-muted-foreground">Total barang</p><p className="mt-1 text-xl font-bold">{pagination.total}</p></div>
        </div>
      </div>

      <div className="rounded-2xl border bg-card p-4 shadow-[0_8px_24px_rgba(28,36,52,0.04)] sm:p-5">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
          <div><h3 className="text-xl font-bold">Isi Rak {rack.code}</h3><p className="mt-1 text-sm text-muted-foreground">Cari nama atau kode barang di Rak {rack.code}.</p></div>
        ...
        <div className="mt-4 flex flex-wrap items-center gap-2 border-t pt-4"><span className="text-sm text-muted-foreground">Jenis:</span><select value={searchParams.get("type") ?? ""} onChange={(event) => updateParams({ type: event.target.value || null })} className="h-9 rounded-lg border bg-background px-3 text-sm"><option value="">Semua barang</option><option value="consumable">Consumable</option><option value="inventory">Alat inventaris</option><option value="unclassified">Perlu klasifikasi</option></select>{searchParams.get("search") && <Button type="button" variant="ghost" size="sm" onClick={() => updateParams({ search: null })}>Hapus pencarian</Button>}</div>
      </div>
      ...
      {records.length === 0 ? (
        ...
        <>
          ...
          <div className="flex flex-col gap-3 text-sm text-muted-foreground sm:flex-row sm:items-center sm:justify-between"><p>Menampilkan {firstRecord}-{lastRecord} dari {pagination.total} barang di Rak {rack.code}</p><div className="flex items-center gap-2"><Button type="button" variant="outline" size="sm" disabled={pagination.page <= 1} onClick={() => changePage(pagination.page - 1)}>Sebelumnya</Button><span>Halaman {pagination.page} dari {totalPages}</span><Button type="button" variant="outline" size="sm" disabled={pagination.page >= totalPages || pagination.total === 0} onClick={() => changePage(pagination.page + 1)}>Berikutnya</Button></div></div>
        </>
      )}
    </section>
  );
