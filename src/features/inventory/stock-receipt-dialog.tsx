  ...
  return (
    <>
      <Button type="button" className="rounded-xl" onClick={() => setOpen(true)}>
        <PackagePlus />Barang Masuk
      </Button>
      <FormDialog open={open} onOpenChange={(nextOpen) => { setOpen(nextOpen); if (!nextOpen) reset(); }} title="Barang Masuk" description="Stok, laporan, dan papan informasi diperbarui otomatis setelah disimpan.">
        <form className="grid gap-4" onSubmit={submit}>
          <div className="grid grid-cols-2 rounded-xl bg-muted p-1">
            <button type="button" onClick={() => setMode("existing")} className={cn("rounded-lg px-3 py-2 text-sm font-semibold", mode === "existing" && "bg-card text-primary shadow-sm")}>Tambah stok</button>
            <button type="button" onClick={() => setMode("new")} className={cn("rounded-lg px-3 py-2 text-sm font-semibold", mode === "new" && "bg-card text-primary shadow-sm")}>Barang baru</button>
          </div>
          {mode === "existing" ? (
            <>
              <Field label="Cari barang yang sudah ada">
                ...
                {selectedItem && <p className="rounded-xl bg-primary/5 p-3 text-sm">Dipilih: <strong>{selectedItem.name}</strong> ({selectedItem.code}, Rak {selectedItem.rackCode})</p>}
              </Field>
              <Field label="Jenis barang">
                <p className="rounded-xl border bg-muted/50 px-3 py-2.5 text-sm">{selectedItem ? itemTypeLabels[selectedItem.itemType] : "Pilih barang terlebih dahulu"}</p>
              </Field>
            </>
          ) : (
            <>
              <Field label="Nama barang"><Input value={name} onChange={(event) => setName(event.target.value)} required /></Field>
              <Field label="Kode barang (opsional)"><Input value={code} onChange={(event) => setCode(event.target.value)} placeholder="Otomatis dibuat bila dikosongkan" /></Field>
              <Field label="Rak">
                <select value={rackId} onChange={(event) => setRackId(event.target.value)} required className="h-10 rounded-xl border bg-background px-3">
                  <option value="">Pilih rak</option>
                  {racks.map((rack) => <option key={rack.id} value={rack.id}>Rak {rack.code}</option>)}
                </select>
              </Field>
              <Field label="Jenis barang">
                <select value={itemType} onChange={(event) => setItemType(event.target.value as "inventory" | "consumable")} className="h-10 rounded-xl border bg-background px-3">
          ...
          <Field label="Sumber atau catatan (opsional)">
            <textarea className="min-h-20 rounded-xl border bg-background px-3 py-2" value={notes} onChange={(event) => setNotes(event.target.value)} placeholder="Contoh: pembelian Juli 2026" />
          </Field>
